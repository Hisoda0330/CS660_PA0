...
private:
  ...
  std::unordered_map<size_t, std::unique_ptr<Page>> m_pagesMap;
  std::list<size_t> m_recentUsed;
  std::unordered_map<size_t, bool> m_dirtyMap;

  std::unordered_map<size_t, PageId> m_hashMap;

  const size_t pageIdToSize_t(const PageId &pid) const;
